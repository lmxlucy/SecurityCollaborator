    ...
    current_user.user_apps.each do |user_app|
      ...
      if user_app.q4 == 'Sometimes' and !user_app.q4_improved
        if !@message.reminders.include? "You are doing well, but you may want to update your " + App.find(user_app.app_id).name + " password a bit more often (eg. every 30, 60, or 90 days)"
          @message.reminders << "You are doing well, but you may want to update your " + App.find(user_app.app_id).name + " password a bit more often (eg. every every 30, 60, or 90 days)"  
        end
      elsif user_app.q4 == 'Rarely'  and !user_app.q4_improved
        if !@message.reminders.include? "You have rooms to improve that you may need to update your " + App.find(user_app.app_id).name + " password a bit more often (eg. every every 30, 60, or 90 days)"
          @message.reminders << "You have rooms to improve that you may need to update your " + App.find(user_app.app_id).name + " password a bit more often (eg. every every 30, 60, or 90 days)"
        end  
